/**
 * Managed, Enhanced & Unique QR Code Scanner
 * - Camera scan (with switch, flashlight), file upload, drag-drop
 * - Scan history, share/copy, WiFi/vCard parsing, smart actions
 * - Unique: scan FPS, scan count, QR info, advanced type detection, custom UI
 */
class QRCodeScanner {
    constructor() {
        this.video = null;
    ...
    async getCameraDevices() {
        try {
            ...
            if (this.availableCameras.length <= 1) switchBtn.style.display = 'none';
        } catch (error) { console.warn('Could not enumerate camera devices:', error); }
    }
    ...
    async toggleFlashlight() {
        ...
        if (track && track.getCapabilities && track.getCapabilities().torch) {
            ...
            try {
                await track.applyConstraints({ advanced: [{ torch: this.flashlightOn }] });
            } catch (err) { alert('Flashlight not supported on this device'); }
        } else { alert('Flashlight not supported on this device'); }
    }
    ...
    scanFrame() {
        ...
        if (qrCode) { this.handleQRDetected(qrCode); }
    ...
    processImageFile(file) {
        ...
        reader.onload = (e) => { this.scanImageData(e.target.result); };
    ...
    scanImageData(imageSrc) {
        ...
        img.onload = () => {
            ...
            const qrCode = jsQR(imageData.data, imageData.width, imageData.height);
            if (qrCode) { this.handleQRDetected(qrCode); }
            else { this.showError('No QR code found in the image'); }
        };
        img.src = imageSrc;
    }
        ...
        resultDiv.innerHTML = `
            <div class="space-y-4">
                <div class="flex items-start space-x-3">
                    <div class="flex-shrink-0"><i class="fas fa-qrcode text-2xl text-green-600"></i></div>
    ...
    showQRInfo(qrCode) {
        ...
        document.getElementById('qr-size-info').textContent = `${qrCode.location ? Math.abs(qrCode.location.topRightCorner.x - qrCode.location.topLeftCorner.x) : 'Unknown'}px`;
    ...
    detectQRType(data) {
        if (/^(http|https):\/\//i.test(data)) return 'URL';
        ...
        if (/^MECARD:/i.test(data)) return 'vCard (MECARD)';
    ...
    analyzeQRType(data) {
        ...
        if (/^(http|https):\/\//i.test(data)) {
            actions.push({icon:'fas fa-external-link-alt',text:'Open URL',action:`window.open('${data}','_blank')`});
        } else if (/^mailto:/i.test(data)) {
            actions.push({icon:'fas fa-envelope',text:'Send Email',action:`window.open('${data}')`});
        } else if (/^tel:/i.test(data)) {
            actions.push({icon:'fas fa-phone',text:'Call Number',action:`window.open('${data}')`});
        } else if (/^sms:/i.test(data)) {
            actions.push({icon:'fas fa-sms',text:'Send SMS',action:`window.open('${data}')`});
        } else if (/^WIFI:/i.test(data)) {
            actions.push({icon:'fas fa-wifi',text:'Show WiFi Info',action:`showWiFiInfo('${this.escapeHtml(data)}')`});
        } else if (/^geo:/i.test(data)) {
            const coords = data.replace('geo:', '').split(',');
            if (coords.length >= 2) {
                actions.push({icon:'fas fa-map-marker-alt',text:'Open in Maps',action:`window.open('https://maps.google.com/?q=${coords[0]},${coords[1]}', '_blank')`});
            }
        } else if (/^BEGIN:VCARD/i.test(data) || /^MECARD:/i.test(data)) {
            actions.push({icon:'fas fa-id-card',text:'Save vCard',action:`showVCardInfo(\`${this.escapeHtml(data)}\`)`});
        }
        actions.push({icon:'fas fa-search',text:'Search Google',action:`window.open('https://www.google.com/search?q=${encodeURIComponent(data)}','_blank')`});
    ...
    addToHistory(data) {
        ...
        if (this.scanHistory.length > 20) { this.scanHistory = this.scanHistory.slice(0, 20); }
        this.saveHistory();
        this.renderHistory();
    }
    ...
    renderHistory() {
        ...
        if (this.scanHistory.length === 0) { historySection.classList.add('hidden'); return; }
    ...
    displayHistoryItem(data) {
        const mockQR = { data: data, version: 'History', errorCorrectionLevel: 'Unknown' };
    ...
    saveHistory() {
        ...
        } catch (error) { console.warn('Could not save scan history:', error); }
    }
    ...
    updateUI() { this.renderHistory(); }

    showError(message) {
        const resultDiv = document.getElementById('scan-result');
        resultDiv.innerHTML = `<div class="text-center text-red-500"><i class="fas fa-exclamation-triangle text-4xl mb-4"></i><p>${message}</p></div>`;
    }
...
function startCamera() { qrScanner.startCamera(); }
function stopCamera() { qrScanner.stopCamera(); }
function switchCamera() { qrScanner.switchCamera(); }
function toggleFlashlight() { qrScanner.toggleFlashlight(); }
function clearHistory() { qrScanner.clearHistory(); }
function removeHistoryItem(id) { qrScanner.removeHistoryItem(id); }
function copyToClipboard(text) {
    if (navigator.clipboard) {
        navigator.clipboard.writeText(text).then(() => { alert('Copied to clipboard!'); }).catch(err => { console.warn('Could not copy text:', err); });
    }
}
function shareQRData(data) {
    if (navigator.share) {
        navigator.share({ title: 'QR Code Data', text: data }).catch(err => console.warn('Could not share:', err));
...
function showVCardInfo(vcardString) {
    alert("vCard Data:\n" + vcardString);
}
